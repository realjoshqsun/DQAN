...
def load_w2v_2nd(embedding_dim, embedding_dim_pos, train_file_path, embedding_path):
    print('\nload embedding...')
    ...
    embedding_pos1 = [list(np.zeros(embedding_dim_pos))]
    embedding_pos1.extend( [list(np.random.normal(loc=0.0, scale=0.1, size=embedding_dim_pos)) for i in range(200)] )
    embedding_pos2 = [list(np.zeros(embedding_dim_pos))]
    embedding_pos2.extend( [list(np.random.normal(loc=0.0, scale=0.1, size=embedding_dim_pos)) for i in range(200)] )

    embedding, embedding_pos1, embedding_pos2 = np.array(embedding), np.array(embedding_pos1), np.array(embedding_pos2)
    
    print("embedding.shape: {} embedding_pos.shape: {}".format(embedding.shape, embedding_pos1.shape))
    print("load embedding done!\n")
    return word_idx_rev, word_idx, embedding, embedding_pos1, embedding_pos2
...
def load_data_2nd_step_new(input_file, word_idx, max_doc_len = 75, max_sen_len = 45):
    print('load data_file: {}'.format(input_file))
    pair_id_all, pair_id, y, x, pair, sen_len, doc_len, related_dis1, related_dis2 = [], [], [], [], [], [], [], [], []
    ...
    while True:
        ...
        for i in pos_list:
            for j in cause_list:
                ...
                entity1, entity2 = i-1, j-1
                pos1, pos2 = [], []
                for a in range(max_doc_len):
                    pos1.append(a - entity1 + 100)
                    pos2.append(a - entity2 + 100)
                related_dis1.append(pos1)
                related_dis2.append(pos2)

    y, x, pair, sen_len, doc_len, related_dis1, related_dis2 = map(np.array, [y, x, pair, sen_len, doc_len, related_dis1, related_dis2])
    for var in ['y', 'x', 'sen_len', 'doc_len']:
        print('{}.shape {}'.format( var, eval(var).shape ))
    print('n_cut {}, (y-negative, y-positive): {}'.format(n_cut, y.sum(axis=0)))
    print('load data done!\n')
    return pair_id_all, pair_id, y, x, pair, sen_len, doc_len, related_dis1, related_dis2
...
def prf_2nd_step(pair_id_all, pair_id, pred_y, fold = 0, save_dir = ''):
    ...
    for i in range(len(pair_id)):
        if pred_y[i]:
            pair_id_filtered.append(pair_id[i])     # pred positive
    def write_log():
        pair_to_y = dict(zip(pair_id, pred_y))
        g = open(save_dir+'pair_log_fold{}.txt'.format(fold), 'w', encoding='UTF-8')
    ...
    o_acc_num = len(s1 & s2)
    acc_num = len(s1 & s3)      # true positive
