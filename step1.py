...
tf.app.flags.DEFINE_float('weight', 4, 'weight of weighted loss')
...
def run():
    ...
    loss_pos = - tf.reduce_sum(y_position * tf.log(pred_pos) * tf.reshape([1, FLAGS.weight], (1, -1))) / valid_num
    loss_cause = - tf.reduce_sum(y_cause * tf.log(pred_cause) * tf.reshape([1, FLAGS.weight], (1, -1))) / valid_num
...
def main(_):
    ...
    FLAGS.scope='P_emotion_weight_' + str(FLAGS.weight) + '_1'
    run()
